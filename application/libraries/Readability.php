...
class Readability
{
     ...
     * @param
     *            string (optional) URL associated with HTML (used for footnotes)
     * @param
     *            string which parser to use for turning raw HTML into a DOMDocument (either 'libxml' or 'html5lib')
     */
    function __construct($data)
    {
        $html = isset($data['html']) ? $data['html'] : null;
        $url = isset($data['url']) ? $data['url'] : null;
        $parser = isset($data['parser']) ? $data['parser'] : 'libxml';
    ...
    function demo()
    {
        ...
        $url = 'http://news.xinhuanet.com/politics/2015-01/03/c_1113857022.htm';
            $html = file_get_contents($url);
            $data = array(
                'html' => $html,
                'url' => $url
            );
            $this->load->library('Readability', $data);
    ...
    protected function getArticleTitle()
    {
        ...
        try {
            $curTitle = $origTitle = $this->getInnerText($this->dom->getElementsByTagName('title')
                ->item(0));
        } catch (Exception $e) {}
     ...
     * @param DOMElement $e            
     * @param
     *            string - what to count. Default is ","
     ...
     * Uses regular expressions to tell if this
    ...
    public function cleanConditionally($e, $tag)
    {
        ...
        for ($i = $curTagsLength - 1; $i >= 0; $i --) {
            ...
            $this->dbg('Cleaning Conditionally ' . $tagsList->item($i)->tagName . ' (' . $tagsList->item($i)
                ->getAttribute('class') . ':' . $tagsList->item($i)
                ->getAttribute('id') . ')' . (($tagsList->item($i)
                ->hasAttribute('readability')) ? (' with score ' . $tagsList->item($i)
